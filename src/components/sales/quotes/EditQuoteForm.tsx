...
import type { Quote, Product } from '../../../types/sales';
...
export default function EditQuoteForm({ quote, isOpen, onClose, onUpdate }: EditQuoteFormProps) {
  const [formData, setFormData] = React.useState({
    id: quote.id,
    number: quote.number,
    date: quote.date,
    validUntil: quote.validUntil?.split('T')[0] || new Date().toISOString().split('T')[0],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.put(`http://localhost:3000/api/quotes/${quote.id}`, formData);
      
      // Si la réponse contient directement les données du devis mis à jour
      if (response.data && (response.data.id || response.data.number)) {
        onUpdate(response.data);
        onClose();
      }
      // Si la réponse contient les données dans un sous-objet dolibarr ou erp
        ...
        setError('Format de réponse inattendu lors de la mise à jour du devis');
      }
    } catch (error: any) {
      console.error('Error updating quote:', error);
      setError(error.response?.data?.message || error.message || 'Une erreur est survenue lors de la mise à jour du devis');
    }
  };
          ...
          <Dialog.Panel className="mx-auto w-full max-w-4xl rounded-xl bg-white">
            <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
              <Dialog.Title className="text-xl font-semibold text-gray-900">
                Modifier le devis {formData.number}
              </Dialog.Title>
              <button
                onClick={onClose}
